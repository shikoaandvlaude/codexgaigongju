...
class RealValidator:
    ...
    def __init__(self, http_engine: HttpEngine, config: dict = None):
        ...
        self.reproduction_attempts = self.config.get(
            "reproduction_attempts",
            validation_config.get("reproduction_attempts", 5),
        )
        self.min_confidence = self.config.get(
            "min_confidence",
            validation_config.get("min_confidence", 0.7),
        )
    ...
    async def validate(self, finding: dict) -> ValidationResult:
        ...
        if result.is_valid and result.confidence < float(self.min_confidence):
            result.is_valid = False
            result.evidence = f"{result.evidence}; confidence below min_confidence={self.min_confidence}"
