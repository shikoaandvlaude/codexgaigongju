...
MAX_OBS_CHARS    = int(os.environ.get("AGENT_MAX_OBS_CHARS", "1800"))
MAX_CTX_CHARS    = int(os.environ.get("AGENT_MAX_CTX_CHARS", "12000"))
MAX_FINDINGS_LOG = int(os.environ.get("AGENT_MAX_FINDINGS_LOG", "120"))
MEMORY_REFRESH_N = int(os.environ.get("AGENT_MEMORY_REFRESH_N", "5"))
RECENT_OBS_N     = int(os.environ.get("AGENT_RECENT_OBS", "2"))
...
class ReActAgent:
    ...
    def _build_context(self) -> str:
        ...
        ctx_parts = [
            ...
            f"Elapsed {elapsed_mins}m/{budget_mins}m | Remaining {remaining}m",
            "",
            f"Completed ({len(completed)}):",
            ", ".join(completed) if completed else "(none yet)",
            "",
            "Notes:",
            self.memory.working_memory or "(empty)",
            "",
            "Findings:",
            self.memory.findings_summary(),
            "",
            "Recent outputs (last 2):",
            self.memory.recent_observations(RECENT_OBS_N),
        ]
        return "\n".join(ctx_parts)
